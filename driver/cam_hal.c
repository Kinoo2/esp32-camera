...
static esp_err_t (*cam_apply_res_params)(void* instance);
static void* cam_apply_res_params_instance = NULL;

static const uint32_t JPEG_SOI_MARKER = 0xFFD8FF; // written in little-endian
                                                  // for esp32
static const uint16_t JPEG_EOI_MARKER = 0xD9FF; // written in little-endian for
                                                // esp32
...
void IRAM_ATTR ll_cam_send_event(cam_obj_t* cam,
      ...
      case CAM_STATE_READ_BUF:
      {
        ...
        if (cam_event == CAM_IN_SUC_EOF_EVENT) {
          ...
          if (cnt || !cam_obj->jpeg_mode || cam_obj->psram_mode) {
            ...
            // send frame
            frame_buffer_event->previous_fov = previousFov;
            frame_buffer_event->fov          = fov;
            previousFov                      = fov;
            fov                              = newFov;
            frame_buffer_event->frame_num    = frame_num++;
            if (!cam_obj->frames[frame_pos].en
                && xQueueSend(
                     cam_obj->frame_buffer_queue, (void*)&frame_buffer_event, 0)
                     != pdTRUE)
            {
...
static lldesc_t* allocate_dma_descriptors(uint32_t count,
                                          uint16_t size,
                                          uint8_t* buffer) {
  lldesc_t* dma =
...
static esp_err_t cam_dma_config(const camera_config_t* config) {
  ...
  cam_obj->dma_node_cnt =
    (cam_obj->dma_buffer_size) / cam_obj->dma_node_buffer_size; // Number of DMA
                                                                // nodes
  cam_obj->frame_copy_cnt =
    cam_obj->recv_size / cam_obj->dma_half_buffer_size; // Number of interrupted
                                                        // copies, ping-pong
                                                        // copy
  ...
  ESP_LOGI(TAG,
           ...
           (int)cam_obj->dma_half_buffer_size,
           (int)cam_obj->dma_node_buffer_size,
           (int)cam_obj->dma_node_cnt,
  ...
  for (int x = 0; x < cam_obj->frame_cnt; x++) {
    ...
    ESP_LOGI(TAG,
             "Allocating %d Byte frame buffer in %s",
             alloc_size,
...
#endif
    CAM_CHECK(cam_obj->frames[x].fb.buf != NULL,
    ...
    if (cam_obj->psram_mode) {
      ...
      ESP_LOGI(TAG,
               "Frame[%d]: Offset: %u, Addr: 0x%08X",
               x,
               cam_obj->frames[x].fb_offset,
               (unsigned)cam_obj->frames[x].fb.buf);
      cam_obj->frames[x].dma =
        allocate_dma_descriptors(cam_obj->dma_node_cnt,
                                 cam_obj->dma_node_buffer_size,
                                 cam_obj->frames[x].fb.buf);
      CAM_CHECK(
        cam_obj->frames[x].dma != NULL, "frame dma malloc failed", ESP_FAIL);
    }
...
esp_err_t cam_init(const camera_config_t* config) {
  ...
  cam_apply_res_params          = config->apply_res_params;
  cam_apply_res_params_instance = config->apply_res_params_instance;
...
esp_err_t cam_config(const camera_config_t* config,
  ...
  ret = ll_cam_set_sample_mode(cam_obj,
                               (pixformat_t)config->pixel_format,
                               config->xclk_freq_hz,
                               sensor_pid);
  ...
  CAM_CHECK_GOTO(
    cam_obj->event_queue != NULL, "event_queue create failed", err);
  ...
  CAM_CHECK_GOTO(cam_obj->frame_buffer_queue != NULL,
                 "frame_buffer_queue create failed",
                 err);
  ...
  xTaskCreatePinnedToCore(cam_task,
                          "cam_task",
                          CAM_TASK_STACK,
                          NULL,
...
camera_fb_t* cam_take(TickType_t timeout) {
  ...
  if (dma_buffer) {
             ...
             && cam_obj->in_bytes_per_pixel != cam_obj->fb_bytes_per_pixel)
    {
      ...
      dma_buffer->len = ll_cam_memcpy(
        cam_obj, dma_buffer->buf, dma_buffer->buf, dma_buffer->len);
    }
    return dma_buffer;
  }
  else {
